...
for subdir, dirs, files in os.walk(imagesdir):
	...
	if(os.path.relpath(os.path.dirname(subdir),imagesdir) == "."):
		...
		f.write("\n\n\n<li class=\"brand\"><a data-toggle=\"collapse\" data-target=\"#"+os.path.relpath(subdir,os.path.dirname(subdir))+"\">"+os.path.relpath(subdir,os.path.dirname(subdir))+" "+"<span class=\"badge pull-right\">85</span></a>\n\n<ul id=\""+os.path.relpath(subdir,os.path.dirname(subdir))+"\" class=\"collapse\">\n\n\n")
		li_flag = True
		...
		if(dirs):
			f.write("\n\n<li><a data-toggle=\"collapse\" data-target=\"#"+os.path.relpath(subdir,os.path.dirname(subdir)).replace(" ", "")+"\"><b>"+os.path.relpath(subdir,os.path.dirname(subdir))+"</b><span class=\"glyphicon glyphicon-option-horizontal pull-right\"></span></a>\n<ul id=\""+os.path.relpath(subdir,os.path.dirname(subdir)).replace(" ", "")+"\" class=\"collapse\">\n")
			ul_flag = True
		else:
